...
async def set_stat(ctx, who, amount, stat): # internal call of set command
	...
	update_to_sheets(get_sheet_id(who), cell, [[int(amount)]])
...
class Sheets(commands.Cog):
	...
	async def get(self, ctx, who, stat, help="See a stat value."):
		...
		await ctx.send(get_from_sheets(get_sheet_id(who), cell))

	async def save(self, ctx, player, help="Save changes to character sheet, or 'all' for everyone."):
		if player == 'all':
			for p in get_player_names():
				await save(self, ctx, p)
		else:
			e = db.find(player)
			for stat in STATS.keys():
				await set_stat(ctx, player, getattr(e, stat), stat)
		
	async def load(self, ctx, player, help="Load a character sheet, or 'all' for everyone."):
		if player == 'all':
			for p in get_player_names():
				await load(self, ctx, p)
		else:
			e = db.find(player)
			for stat in STATS.keys():
				setattr(e, stat, await get(self, ctx, player, stat))
